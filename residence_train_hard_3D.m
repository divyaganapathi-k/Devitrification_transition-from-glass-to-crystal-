...
phop_all=sortrows(phop_all,[6 5]);
...
%hard particles
phop_required=horzcat(phop_required,diff(:,5:6));
f=(phop_required(:,8)==0 & phop_required(:,5)>0 & phop_required(:,5)<=2000);
phop_required=phop_required(f,:);
phop_required=sortrows(phop_required,-7);
phop_hard=phop_required(1:600,:);
phop_hard=phop_hard(:,1:6);
% phop_hard(:,6)=-1;
%phop_hard contains dedrifted coordinates
% clear phop_all
% load('E:\Devitrification\Sample5\Sample5_analysis\Sample5_track(28).mat');
[~,ia,ib]=intersect(phop_hard(:,2:6),H,'rows');
req_coord_hard=horzcat(R(ib,1:3),phop_hard(ia,5:6));
% clear H1 R
% load('E:\Devitrification\Sample5\Sample5_analysis\Sample5_radial_density_set1_part1.mat');
[~,ia,ib]=intersect(req_coord_hard(:,1:4),radial_density(:,1:4),'rows');
